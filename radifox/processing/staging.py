...
class Staging(ProcessingModule):
    ...
    def run(
        ...
        session_imgs = {}
        for all_imgs, img_filters, best_res, proc_plugin_paths, skip_defaults in zip(
                session_filepaths, image_types, keep_best_res, plugin_paths, skip_default_plugins
        ):
            ...
            out_imgs = [fix_sform_qform(img) for img in filtered_imgs]

            # Remove staged images that are not in the filtered image list
            for stage_img in (session / "stage").iterdir():
                if stage_img.name not in [img.name for img in out_imgs]:
                    stage_img.unlink()
...
class MP2RAGEPlugin(StagingPlugin):
    ...
    def create_uniden(imgs: list[ImageFile], gamma: float = 1e9) -> ImageFile:
        ...
        for img in imgs:
            ...
            cmplx_type = next(
                (ct for ct in MP2RAGEPlugin.CMPLX_IMG_TYPES if ct in img.extras),
                "MAG"
            )
