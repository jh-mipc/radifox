...
from nibabel.volumeutils import Recoder, array_from_file
from nibabel.affines import from_matvec, dot_reduce, apply_affine
from nibabel.nifti1 import unit_codes
from nibabel.fileslice import fileslice, strided_scalar
...
image_def_dtds = {'V3': []}
image_def_dtds['V4'] = image_def_dtds['V3'] + [
    ('slice_number', int),
    ('echo_number', int,),
    ('dynamic_scan_number', int,),
    ('cardiac_phase_number', int,),
    ('image_type_mr', int,),
    ('scanning_sequence', int,),
    ('index_in_rec_file', int,),
    ('image_pixel_size', int,),
    ('scan_percentage', int,),
    ('recon_resolution', int, (2,)),
    ...
    ('window_center', float,),
    ('window_width', float,),
    ('image_angulation', float, (3,)),
    ('image_offcentre', float, (3,)),
    ('slice_thickness', float),
    ('slice_gap', float),
    ('image_display_orientation', int,),
    ('slice_orientation', int,),
    ('fmri_status_indication', int,),
    ('image_type_ed_es', int,),
    ('pixel_spacing', float, (2,)),
    ('echo_time', float),
    ...
    ('minimum_rr_interval', int,),
    ('maximum_rr_interval', int,),
    ('turbo_factor', int,),
    ('inversion_delay', float)]

# Extra image def fields for 4.1 compared to 4
image_def_dtds['V4.1'] = image_def_dtds['V4'] + [
    ('diffusion_b_value_number', int,),     # (imagekey!)
    ('gradient_orientation_number', int,),  # (imagekey!)
    ('contrast_type', int),               # Any non-int in the wild?
    ('diffusion_anisotropy_type', int),   # Any non-int in the wild?
    ('diffusion', float, (3,)),
]

# Extra image def fields for 4.2 compared to 4.1
image_def_dtds['V4.2'] = image_def_dtds['V4.1'] + [
    ('label_type', int,),                  # (imagekey!)
]

# Extra image def fields for alternate v4.2 after R5.4
v42_alt_defs = [
    ('contrast_bolus_agent', 'S30'),
    ('contrast_bolus_route', 'S30'),
    ('contrast_bolus_volume', float),
    ('contrast_bolus_start_time', 'S30'),
    ('contrast_bolus_total_dose', float),
    ('contrast_bolus_ingredient', 'S30'),
    ('contrast_bolus_ingredient_concentration', float),
]
image_def_dtds['V4.2ALT'] = image_def_dtds['V4.2'] + v42_alt_defs
...
class TruncatedPARRECError(PARRECError):
    pass
...
def _split_header(fobj):
    ...
    for line in fobj:
        ...
        if state == 'comment-block':
            if 'Contrast Bolus Agent' in line and version == 'V4.2':
                version = 'V4.2ALT'
            if not line.startswith('#'):
                state = 'image-info'
        if state == 'image-info':
...
def _process_image_lines(image_lines, version):
    ...
    for i, line in enumerate(image_lines):
        ...
        for props in image_def_dtd:
            if version == 'V4.2ALT':
                if props[0] == 'label_type':
                    try:
                        props[1](items[item_counter])
                    except ValueError:
                        split_char = 0
                        for j, char in enumerate(items[item_counter]):
                            if not char.isdigit():
                                split_char = j
                                break
                        temp_item = items[item_counter]
                        items[item_counter] = temp_item[:split_char]
                        items.insert(item_counter + 1, temp_item[split_char:])
                elif props[0] == 'contrast_bolus_agent':
                    if all([items[item_counter + idx] == '0.000000' for idx in range(3)]):
                        break
                elif props[0] == 'contrast_bolus_route':
                    try:
                        float(items[item_counter + 1])
                    except ValueError:
                        items[item_counter] = ' '.join([items[item_counter], items[item_counter + 1]])
                        del items[item_counter + 1]
            if len(props) == 2:
                name, np_type = props
                ...
                raise PARRECError('Invalid image def specification %s' % str(props))
...
def vol_is_full(slice_nos, slice_max, slice_min=1) -> np.ndarray:
...
def _truncation_checks(general_info: dict, image_defs: np.ndarray, permit_truncated: bool) -> None:
    ...
    # _chk_trunc('dynamic_scan', 'max_dynamics')
    # _chk_trunc('diffusion_b_value', 'max_diffusion_values')
    # _chk_trunc('gradient_orientation', 'max_gradient_orient')

    # Final check for partial volumes
    if not np.all(vol_is_full(image_defs['slice_number'], general_info['max_slices'])):
...
class PARRECHeader(SpatialHeader):
    ...
    def __init__(self, info: dict, image_defs: np.ndarray, permit_truncated: bool = False,
        ...
        # functionality
        # dtype
        bitpix = self._get_unique_image_prop('image_pixel_size')
    ...
    @classmethod
    def from_fileobj(cls, fileobj, permit_truncated=False,
                     strict_sort=False):
        info, image_defs = parse_par_header(fileobj)
        return cls(info, image_defs, permit_truncated, strict_sort)
    ...
    def set_max_values(self):
        self.general_info['max_echoes'] = len(set(self.image_defs['echo_number']))
        self.general_info['max_dynamics'] = len(set(self.image_defs['dynamic_scan_number']))
    ...
    def get_voxel_size(self):
        ...
        slice_thickness = self._get_unique_image_prop('slice_thickness')
        voxsize_inplane = self._get_unique_image_prop('pixel_spacing')
    ...
    def _get_n_slices(self):
        ...
        return len(set(self.image_defs['slice_number']))

    def _get_n_vols(self):
        """ Get number of volumes for output data """
        slice_nos = self.image_defs['slice_number']
    ...
    def _calc_data_shape(self):
        ...
        some attributes available in the fully initialized object.
        """
        inplane_shape = tuple(self._get_unique_image_prop('recon_resolution'))
    ...
    def get_data_scaling(self, method="dv"):
        ...
        scale_slope = self.image_defs['scale_slope']
        rescale_slope = self.image_defs['rescale_slope']
        rescale_intercept = self.image_defs['rescale_intercept']
        if method == 'dv':
    ...
    def get_slice_orientation(self):
        ...
        """
        lab = self._get_unique_image_prop('slice_orientation')
    ...
    def _strict_sort_order(self):
        ...
        slice_nos = idefs['slice_number']
        dynamics = idefs['dynamic_scan_number']
        phases = idefs['cardiac_phase_number']
        echos = idefs['echo_number']
        ...
        if self.general_info['diffusion'] != 0:
            bvals = self.get_def('diffusion_b_value_number')
            if bvals is None:
                bvals = self.get_def('diffusion_b_factor')
            bvecs = self.get_def('gradient_orientation_number')
            if bvecs is None:
    ...
    def get_volume_labels(self):
        ...
        dynamic_keys = ['cardiac_phase_number',
                        'echo_number',
                        'label_type',
                        'image_type_mr',
                        'dynamic_scan_number',
        ...
        sl1_indices = image_defs['slice_number'][sorted_indices] == 1
...
class PARRECImage(SpatialImage):
    ...
    @classmethod
    def from_file_map(cls, file_map, *, mmap=True, permit_truncated=False,
        ...
        with file_map['header'].get_prepare_fileobj('rt') as hdr_fobj:
            hdr = cls.header_class.from_fileobj(
                hdr_fobj,
                permit_truncated=permit_truncated,
                strict_sort=strict_sort)
        rec_fobj = file_map['image'].get_prepare_fileobj()
        data = cls.ImageArrayProxy(rec_fobj, hdr, mmap=mmap, scaling=scaling)
        return cls(data, hdr.get_affine(), header=hdr, extra=None, file_map=file_map)

    def to_file_map(self, file_map=None):
        pass

    @classmethod
    def from_filename(cls, filename, *, mmap=True, permit_truncated=False,
                      scaling='dv', strict_sort=False):
        ...
        strict_sort : bool, optional, keyword-only
            ...
            the slices appear in the .PAR file.
        """
        file_map = cls.filespec_to_file_map(filename)
        return cls.from_file_map(file_map,
                                 mmap=mmap,
                                 permit_truncated=permit_truncated,
                                 scaling=scaling,
                                 strict_sort=strict_sort)
