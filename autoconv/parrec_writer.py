...
def array_string_func(format_str, sep=' '):
    return lambda x: sep.join([format_str]*len(x)).format(*tuple(x.squeeze().tolist()) if len(x) > 1 else (x[0],))
...
def gen_dict_strings(format_dict, value_dict_array):
    ...
    return {k: func(value_dict_array.get(k, default)) if callable(func)
...
def split_fix_parrec(in_filename, study_uid, outdir):
    ...
    split_defs = set(zip(zip(*[idefs[def_name].tolist() for def_name in split_types]),
                         slice_vols))
    split_vols = {}
