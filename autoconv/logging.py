...
def create_loggers(output_root, scan_dir, verbose=False):
    mkdir_p(os.path.join(output_root, scan_dir, 'logs'))
    log_formatter = logging.Formatter('%(asctime)s [%(levelname)s] %(message)s')
    log_stream = logging.StreamHandler(sys.stdout)
    log_stream.setFormatter(log_formatter)
    log_file = logging.FileHandler(os.path.join(output_root, scan_dir,
    ...
    warn_file = logging.FileHandler(os.path.join(output_root, scan_dir,
    ...
    error_file = logging.FileHandler(os.path.join(output_root, scan_dir,
